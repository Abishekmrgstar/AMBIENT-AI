...
const translations = {
  en: {
    ...
    audioSummary: 'Listen to visit summary',
    audioSummaryDesc: 'Hear a spoken summary of your transcript, medications, and clinical report.',
  ...
  ta: {
    ...
    audioSummary: 'வருகை சுருக்கத்தைக் கேளுங்கள்',
    audioSummaryDesc: 'உங்கள் படியெடுப்பு, மருந்துகள் மற்றும் மருத்துவ அறிக்கையின் பேச்சு சுருக்கம்.',
    playSummary: 'சுருக்கத்தை இயக்கு',
    pauseSummary: 'இடைநிறுத்து',
    stopSummary: 'நிறுத்து',
    playing: 'இயக்குகிறது...',
  ...
  hi: {
    ...
    audioSummary: 'दौरे का सार सुनें',
    audioSummaryDesc: 'आपके प्रतिलेख, दवाइयों और क्लीनिकल रिपोर्ट का ऑडियो सार सुनें।',
    playSummary: 'सार चलाएं',
    pauseSummary: 'रोकें',
    stopSummary: 'बंद करें',
    playing: 'चल रहा है...',
    paused: 'रुका हुआ'
  }
}
...
// Build a single summary string for TTS from visit data (English for consistent speech)
function buildVisitSummaryText(visit) {
  if (!visit) return ''
  const parts = []
  parts.push(`Visit summary for visit number ${visit.id}, dated ${new Date(visit.created_at).toLocaleDateString('en-IN', { day: 'numeric', month: 'long', year: 'numeric' })}.`)
  if (visit.transcript?.content) {
    const transcriptPreview = visit.transcript.content.length > 500 ? visit.transcript.content.slice(0, 500) + '...' : visit.transcript.content
    parts.push('Conversation transcript. ' + transcriptPreview)
  }
  if (visit.clinical_report) {
    const cr = visit.clinical_report
    if (cr.symptoms) parts.push('Symptoms. ' + cr.symptoms)
    if (cr.diagnosis) parts.push('Diagnosis. ' + cr.diagnosis)
    if (cr.medications) parts.push('Medications. ' + cr.medications)
    if (cr.follow_up) parts.push('Follow up. ' + cr.follow_up)
  }
  if (visit.patient_report) {
    const pr = visit.patient_report
    if (pr.diagnosis_summary) parts.push('Diagnosis summary. ' + pr.diagnosis_summary)
    if (pr.medication_instructions) parts.push('Medication instructions. ' + pr.medication_instructions)
    if (pr.warning_signs) parts.push('Warning signs to watch. ' + pr.warning_signs)
    if (pr.content) parts.push('Detailed instructions. ' + pr.content)
  }
  return parts.join(' ')
}
...
export default function PatientDashboard() {
  ...
  // Cancel TTS when visit changes or component unmounts
  ...
  function playSummary() {
    const text = buildVisitSummaryText(displayVisit)
            ...
            <div className="flex-1 p-6 space-y-6 min-w-0">
              {displayVisit && (
                <>
                  ...
                  {/* Audio summary player - end of patient page */}
                  {displayVisit && buildVisitSummaryText(displayVisit) && (
                    <div className="p-5 rounded-xl bg-violet-50/90 border border-violet-200 shadow-sm">
                      <h3 className="font-semibold text-violet-900 mb-1">{t.audioSummary}</h3>
                      <p className="text-sm text-violet-700 mb-4">{t.audioSummaryDesc}</p>
                      <div className="flex flex-wrap items-center gap-3">
                        {!audioPlaying && !audioPaused && (
                          <button
                            type="button"
                            onClick={playSummary}
                            className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-violet-600 text-white font-medium hover:bg-violet-700 focus:ring-2 focus:ring-violet-500 focus:ring-offset-2"
                          >
                        ...
                        {(audioPlaying || audioPaused) && (
                          <>
                            ...
                            >
                              {audioPaused ? (
                                <><svg className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20"><path d="M6.3 2.841A1.5 1.5 0 004 4.11V15.89a1.5 1.5 0 002.3 1.269l9.344-5.89a1.5 1.5 0 000-2.538L6.3 2.84z" /></svg>{t.playSummary}</>
                              ) : (
                                <><svg className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zM7 8a1 1 0 012 0v4a1 1 0 11-2 0V8zm5-1a1 1 0 00-1 1v4a1 1 0 102 0V8a1 1 0 00-1-1z" clipRule="evenodd" /></svg>{t.pauseSummary}</>
                              )}
                            </button>
                            <button
                              type="button"
                              onClick={stopSummary}
                              className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl border-2 border-violet-300 bg-white text-violet-700 font-medium hover:bg-violet-50 focus:ring-2 focus:ring-violet-500 focus:ring-offset-2"
                            >
                              <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8 7a1 1 0 012 0v6a1 1 0 11-2 0V7z" clipRule="evenodd" /></svg>
                              {t.stopSummary}
                            </button>
                            <span className="text-sm text-violet-600 font-medium">{audioPaused ? t.paused : t.playing}</span>
                          </>
                        )}
                      </div>
                    </div>
